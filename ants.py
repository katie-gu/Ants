...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        range_bee = 0
        while range_bee < self.min_range and current_place.entrance != hive:
            range_bee += 1
            current_place = current_place.entrance
        bees_list = list(current_place.bees)
        while len(bees_list) == 0 and range_bee <= self.max_range and current_place != hive:
            current_place = current_place.entrance
            range_bee += 1
            bees_list = list(current_place.bees)
        if len(bees_list) != 0 and range_bee <= self.max_range and current_place != hive:
            bees_list = list(current_place.bees)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.is_imposter:
            ...
            while place:
                if place.ant not in self.doubled_damage:
                    if isinstance(place.ant, FireAnt) or isinstance(place.ant, NinjaAnt) or isinstance(place.ant, ThrowerAnt):
                        place.ant.damage *= 2
                        self.doubled_damage.append(place.ant)
                    elif isinstance(place.ant, BodyguardAnt):
                        if isinstance(place.ant, TankAnt):
                            place.ant.damage *= 2
                        if place.ant.ant is not None and place.ant.ant not in self.doubled_damage:
                            place.ant.ant.damage *= 2
                            self.doubled_damage.append(place.ant.ant)
                        self.doubled_damage.append(place.ant)
                else:
                    if isinstance(place.ant, BodyguardAnt) and place.ant.ant is not None and place.ant.ant not in self.doubled_damage:
                        place.ant.ant.damage *= 2
                        self.doubled_damage.append(place.ant.ant)
                place = place.exit
